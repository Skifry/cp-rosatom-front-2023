<script>
    ...
    let map = new Image(undefined, undefined); map.src = "/static/maps_yamal.svg"
    ...
    const drawMap = function () {
        ...
        docks.forEach(dock => {
            ...
            if (zoom >= 3) {
                ctx.fillStyle = "#000"
                ctx.fillText(dock.name, (dock.x - ox) * zoom - 14, (dock.y - oy) * zoom - 20)
            }
        })
        ships.forEach(ship => {
        ...
        ctx.fillStyle = "#eee"
        ctx.fillRect(ctx.canvas.width - 115, ctx.canvas.height - 20, 115, 20)
        ctx.fillStyle = "#000"
        ctx.fillText("Росатомфлот СМП", ctx.canvas.width - 110, ctx.canvas.height - 5)
    }
    ...
    export const latLongToMap = function (latitude, longitude) {
    ...
    const changeOffset = function (nox, noy) {
        ...
        ox = Math.max(25, Math.min(pix, nox))
        let piy = imgHeight - (width / aspectRatio / zoom);
        oy = Math.max(90, Math.min(piy, noy))
    }
    ...
    export const addDockXY = function (x, y, name, onHover, onClick) {
        docks.push({x, y, name, onHover, onClick})    
        drawMap()
    }

    export const addDockLatLon = function (lat, lon, name, onHover, onClick) {
        let coords = latLongToMap(lat, lon)
        addDockXY(coords.x, coords.y, name, onHover, onClick)    
    }
